...
def cutadapt(f1):
    ...
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
    fo_1 = out_dir + os.path.basename(f1)
    fo_2 = out_dir + os.path.basename(f2)
    ##illumina universal adapter
    #cmd = 'cutadapt -a AGATCGGAAGAG -A AGATCGGAAGAG -m 70 -o {} -p {} {} {}'.format(fo_1, fo_2, f1, f2)

    ##nextera transposase sequence
    cmd = 'cutadapt -a CTGTCTCTTATA -A CTGTCTCTTATA -m 70 -o {} -p {} {} {}'.format(fo_1, fo_2, f1, f2)

    ##illumina small RNA 3'
    #cmd = 'cutadapt -a ATGGAATTCTCG -A GATCGTCGGACT --discard-untrimmed --minimum-length=16 --maximum-length=30 -o {} -p {} {} {}'.format(fo_1,fo_2, f1,f2)
    print cmd
    os.system(cmd)
...
### cut adapter
fs = glob.glob('fastq/*/*R1.fastq.gz')
Parallel(n_jobs=3)(delayed(cutadapt)(f) for f in fs_new)
